...
echo ■ 진단 기준
echo 	양호 : “계정 잠금 기간” 및 “계정 잠금 기간 원래대로 설정 기간”이 설정됨. (60분 이상 권고)
echo 	취약 : “계정 잠금 기간” 및 “잠금 기간 원래대로 설정 기간”이 설정 안 됨.
echo.
echo.

echo ■ 진단 결과
call .\module\mod_createSecEdit.bat
for /f "tokens=3" %%A in ('find /i "LockoutBadCount" .\module\tmp\securityPolicy.txt') do (
    set /a "count0=%%A"
)
